...
import dataJson from './family.json'
import './App.css';
...
function App() {
  ...
  var familyDataFilename = ""; // Your own family.txt
  var defaultRootName = 'Raman';                // Someone in your family
  var lineHeight = 280;  // 220 is better, but the Simpsons pngs are very vertical

  var personName = document.getElementById("personName");
  var selectRelationby = document.getElementById("relationby");
  var selectRelationto = document.getElementById("relationto");

  // all entries
  const [entries1, setentries1] = React.useState(dataJson)
  ...
  function relationbySelected() {

    personName = document.getElementById("personName");
    selectRelationby = document.getElementById("relationby");
    selectRelationto = document.getElementById("relationto");

    var selectedValue = selectRelationby.options[selectRelationby.selectedIndex].value;
    var names = Object.keys(entries1).filter(name => !isPerson(name))

    var personNames = Object.keys(entries1).filter(name => isPerson(name))
    selectRelationto.options.length = 0;
    ...
    if (selectedValue === "child") {
      ...
      for (var i = 0; i < names.length; i++) {
        ...
        selectRelationto.add(option);
      }
    }
    else if (selectedValue === "partner") {
      for (var i = 0; i < personNames.length; i++) {
        var option = document.createElement("option");
        option.text = personNames[i];
        selectRelationto.add(option);
      }
    }
  ...
  function addNewPerson() {
    ...
    selectRelationby = document.getElementById("relationby");
    selectRelationto = document.getElementById("relationto");
    var temp = entries1 ? entries1 : {}

    if (personName.value && selectRelationby.value)


      if (selectRelationby.value === "child") {

        temp[selectRelationto.value][0] = temp[selectRelationto.value][0] ? temp[selectRelationto.value][0] + ", " + personName.value : "c: " + personName.value
        temp[personName.value] = []
        asyncWriteBackTextFile(familyDataFilename, temp)

      }
      else if (selectRelationby.value === "partner") {
        var newpair = personName.value + " + " + selectRelationto.value
        temp[newpair] = []
        asyncWriteBackTextFile(familyDataFilename, temp)

      }
      else {
        var newpair = personName.value
        temp[newpair] = []
        asyncWriteBackTextFile(familyDataFilename, temp)
      }
  ...
  function makeDivs(entries, neighbours) {
    var result = {};
    var names = Object.keys(neighbours)

    for (var name of names.filter(n => isPerson(n))) {
  ...
  function asyncWriteBackTextFile(filename, newJson) {
    localStorage.setItem("data", JSON.stringify(newJson))
    getNeighboursInEntries(newJson)
  ...
  React.useEffect(() => {
    ...
    if (JSON.parse(jsonData)) {
      getNeighboursInEntries(JSON.parse(jsonData))
    }
    else {
      localStorage.setItem("data", JSON.stringify(dataJson))
      getNeighboursInEntries(dataJson)
    }
  }, [])

  function getNeighboursInEntries(entries0) {
    var neighbours = getNeighbours(entries0);


    validateTreeStructure(neighbours);
    var divs = makeDivs(entries0, neighbours);

    setentries1(entries0)
    entries = entries0

    // Need to save divs and neighbours, also keep entries for debugging.
    window.state = { entries0, divs, neighbours };
  ...
  return (
    <>
      <button style={{ padding: 10, paddingLeft: 30, paddingRight: 30, margin: 10 }} type="button" onClick={() => setopen(true)}>Add new person</button>
      <Drawer open={open} onClose={() => { setopen(false) }}>
        <div style={{ marginTop: 10 }}>

          <div style={{ marginTop: 10, }}>
            Name of the person -- Give unique name
          </div>
          <div>
            <input style={{ padding: 10, width: 310, marginTop: 5 }} id="personName" type="text" />
          </div>
          <div>
          ...
          <div>
            <select style={{ padding: 10, width: 335, marginTop: 5, }} id="relationby" onChange={e => relationbySelected(e)} aria-placeholder="relation by">
          ...
          <div>
            <select style={{ padding: 10, width: 335, marginTop: 5, }} id="relationto" aria-placeholder="relation to">
            </select>
          </div>
          <div style={{ marginTop: 10 }}>

          </div>
          <button style={{ padding: 10, marginTop: 50, width: 335 }} type="button" onClick={() => addNewPerson()}>Add</button>
        </div>
      </Drawer>
